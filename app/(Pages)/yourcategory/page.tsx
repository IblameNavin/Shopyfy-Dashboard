    ...
    <div className="w-full">
      ...
      <div className="py-4 md:py-6 lg:py-8 md:px-[5vw] xl:px-[11vw] flex flex-col gap-4">
        <div className="flex items-center justify-between">
          ...
          <button className="flex items-center justify-center gap-1.5 bg-green-500 py-2 px-4 text-black font-medium rounded-md cursor-pointer hover:bg-green-600 transition-colors"> 
        ...
        <div className='flex-1 flex flex-col border rounded-md border-white/5 w-full min-h-160 py-4 bg-[#1C1917]'>
          ...
          {/* Controls Box */}
          <div className="px-4 flex flex-col lg:flex-row lg:justify-between gap-4 shrink-0 mb-8 text-white">
            <div className="flex w-full lg:w-[70%] x15:w-[71%] rounded-md px-2 items-center gap-4 border border-white/20 lg:justify-between ">
              <input type="text" placeholder="Search Name" className="text-sm px-1.5 py-2 bg-black text-white rounded-md w-80 outline-none lg:w-full"/>
              <CiSearch size={22} className="text-gray-400 shrink-0"/>
            </div>

            <div className="flex items-center gap-3">
              <Link href={"/category"} className={`py-2 px-4 rounded-md cursor-pointer transition-all duration-300 ease-in-out ${activeGreen === "all" ? "bg-green-500 text-black font-medium" : "bg-black text-gray-400 border border-white/10"}`} onClick={()=>setActiveGreen("all")}>All Categories</Link>
              <button className={`py-2 px-4 rounded-md cursor-pointer transition-all duration-300 ease-in-out ${activeGreen === "your" ? "bg-green-500 text-black font-medium" : "bg-black text-gray-400 border border-white/10"}`} onClick={()=>setActiveGreen("your")}>Your Categories</button>
            </div>
          </div>
          ...
          <div className="w-full overflow-x-auto custom-scrollbar">
            ...
            <div className='min-w-[1100px] xl:min-w-full flex-1 flex flex-col text-white px-4 pb-1'>
              ...
              {clientData?.slice(0, 6).map((item, index) =>{
                return <>
              <div key={index} className={`${tableGridClass} py-6 lg:py-6 px-4 border-b border-white/5 text-gray-300 hover:bg-white/3 transition-colors`}> 
                  <div className='text-left font-medium text-gray-500'>{index + 1}</div>
                  <div className='text-left font-medium text-white'>{item.category}</div>
                  <div className="flex items-center justify-start gap-3 w-full min-w-0">
                    ...
                    <span className='truncate text-left'>{item.superCategory}</span>
                  </div>
                  <div className='text-left font-semibold pl-4'>{item.totalProducts}</div>
