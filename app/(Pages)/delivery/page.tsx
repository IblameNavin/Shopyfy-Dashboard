...
const Delivery = () => {
  ...
  const cardClass = `flex flex-col gap-1 border p-1.5 rounded-md shadow-md flex-1 ${isActive ? "border-black/20 shadow-gray-200" : "border-white/20 shadow-gray-300"}`
  ...
  return (
    <div className="w-full px-3">
      <div className="py-4 md:py-6 lg:py-8 md:px-[5vw] xl:px-[11vw] flex flex-col gap-4">
        ...
        <button className={`py-2 px-4 bg-green-500 text-black rounded-md cursor-pointer w-fit ${isActive ? "text-white" : "text-black"}`}>Save Changes</button>
        <button className={`py-2 px-4 bg-red-500 text-black rounded-md cursor-pointer w-fit ${isActive ? "text-white" : "text-black"}`}>Cancel</button>
        </div>
        </div>


        {/* Table  */}
        <div className={`py-2 px-3 border rounded-md w-full flex flex-col ${isActive ? "bg-white border-black/10 shadow-md text-black" : "bg-[#1C1917] border-white/5 text-white"}`}>
          {/* Inner Div Wrapper */}
